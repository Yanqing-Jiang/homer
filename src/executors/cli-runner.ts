...
const MEMORY_HINT =
  "Use Homer memory MCP tools directly when memory matters. Call memory_context for fresh current context, memory_search for lookup (mode='fetch' to read a durable file whole, mode='hybrid' for chunk-only semantic recall), memory_suggest for decisions/preferences/lessons, and memory_promote only for simple stable facts.";
const TELEGRAM_HINT =
