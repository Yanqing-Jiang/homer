...
/**
 * Check if two titles are semantically similar (likely duplicates)
 * Uses a threshold of 0.8 for high confidence matches
 */
export function areTitlesSimilar(
  ...
  const fpA = createFingerprint(titleA);
  const fpB = createFingerprint(titleB);
  return fingerprintSimilarity(fpA, fpB) >= threshold;
}

/**
 * Find the best matching title from a list
...
export function findBestMatch(
  ...
  const fp = createFingerprint(title);
  let bestMatch: { id: string; title: string; similarity: number } | null = null;

  for (const candidate of candidates) {
    const candidateFp = createFingerprint(candidate.title);
    const similarity = fingerprintSimilarity(fp, candidateFp);
